...
- `v*` tag 触发 CLI 发布到 npm `@yakami129/msctl`
- `v*` tag 触发 iOS 发布到 TestFlight
...
3. 从 tag 名（去掉 `v` 前缀）覆盖 `cli/npm/package.json` 的 `version`
...
- 打 tag 前手动确保 `cli/Cargo.toml` 和 `cli/npm/package.json` 的 `version` 字段一致
- `release-cli.yml` 中从 tag 名（`v1.2.3` → `1.2.3`）覆盖 `cli/npm/package.json` 的 `version`，确保 npm 包版本与 tag 一致
...
- [ ] 打 tag 后，GitHub Releases 出现对应 release，附带 4 个二进制下载
- [ ] 打 tag 后，TestFlight 出现新构建
