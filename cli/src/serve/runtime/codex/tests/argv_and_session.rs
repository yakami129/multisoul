...
///   model_id    = "gpt-5.5"（conversation 级运行时模型选择）
...
///   - 正断言：queued.model_id == Some("gpt-5.5")，说明模型选择进入 worker 队列
...
///
/// 数据构造（含关键数值的推导过程）：
///   selected_model = "gpt-5.5"（conversation.model_id 的具体值）
...
///
/// 执行过程：
///   1. 调用 build_codex_args(..., model_id=Some("gpt-5.5"))
///   2. 分别检查 fresh/resume argv 是否包含 `--model gpt-5.5`
///   3. 再调用 model_id=None 的 fresh argv
///
/// 预期结果：
///   - 正断言：fresh argv 包含 `--model gpt-5.5`
///   - 正断言：resume argv 包含 `--model gpt-5.5`
///   - 负断言：None argv 不包含 `--model`
#[test]
fn test_build_codex_args_includes_selected_model() {
    let fresh = build_codex_args("/repo", None, "full-auto", None, Some("gpt-5.5"));
    let resume = build_codex_args("/repo", Some("thread-1"), "suggest", None, Some("gpt-5.5"));
...
/// build_codex_args: compound model:effort IDs split into --model and -c model_reasoning_effort.
///
/// Data:
///   compound = "gpt-5.5:high" → --model gpt-5.5 -c model_reasoning_effort=high
///   plain    = "gpt-5.5"      → --model gpt-5.5 (no -c flag)
#[test]
fn test_build_codex_args_splits_compound_effort() {
    let with_effort = build_codex_args("/repo", None, "full-auto", None, Some("gpt-5.5:high"));
    assert!(
        with_effort.windows(2).any(|w| w == ["--model", "gpt-5.5"]),
        "compound ID should pass the base model to --model"
    );
    assert!(
        with_effort
            .windows(2)
            .any(|w| w == ["-c", "model_reasoning_effort=high"]),
        "compound ID should pass reasoning effort via -c"
    );

    let without_effort = build_codex_args("/repo", None, "full-auto", None, Some("gpt-5.5"));
    assert!(
        without_effort
            .windows(2)
            .any(|w| w == ["--model", "gpt-5.5"]),
        "plain ID should pass the model to --model"
    );
    assert!(
        !without_effort.iter().any(|a| a == "-c"),
        "plain ID should not include -c effort override"
    );
}
