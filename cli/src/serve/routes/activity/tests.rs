...
async fn get_activity_json(uri: &str) -> serde_json::Value {
    ...
    get_activity_json_from(app, uri).await
}

async fn get_activity_json_from(app: Router, uri: &str) -> serde_json::Value {
    let resp = app
        .oneshot(
            Request::builder()
                .uri(uri)
...
async fn post_activity_status(app: Router, uri: &str, authorized: bool) -> StatusCode {
    let mut builder = Request::builder().method("POST").uri(uri);
    if authorized {
        builder = builder.header("Authorization", "Bearer ms_v2_tok");
    }
    let resp = app
