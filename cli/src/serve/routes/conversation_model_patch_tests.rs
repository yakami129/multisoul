...
async fn test_patch_conversation_model_inserts_system_event() {
    ...
    assert_value(
        ...
        "GPT-5.5",
        "to_label should render gpt-5.5 with the product-facing label",
    );
}
...
///   2. PATCH { "model_id": "gpt-5.5" } with a valid model id
...
///
/// 数据构造（含关键数值的推导过程）：
///   starting model = "gpt-5.5"（concrete override already persisted）
