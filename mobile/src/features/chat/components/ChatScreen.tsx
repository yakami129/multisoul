...
export function ChatScreen({ agentName, messages, status, onSend, onBack }: Props) {
  ...
  const incomingAssistantId =
    isAwaitingResponse && latestAssistant && latestAssistant.id !== lastSeenAssistantIdRef.current
      ? latestAssistant.id
      : null;
  ...
  return (
    ...
    >
      ...
      {status === 'reconnecting' && (
        ...
        >
          <Text
            style={{ color: '#2D8B2D', fontSize: 11, textAlign: 'center', fontFamily: 'Inter' }}
          >
            Reconnecting…
          </Text>
        </View>
      ...
      <FlatList
        ref={listRef}
        data={renderMessages}
        keyExtractor={(m: RenderMessage) => m.id}
        contentContainerStyle={{ padding: 16, paddingBottom: 8 }}
        onContentSizeChange={() => listRef.current?.scrollToEnd({ animated: true })}
        renderItem={({ item }: { item: RenderMessage }) => {
          ...
          return (
            ...
            >
              ...
              >
                ...
                >
                  {getDisplayText(item as RenderMessage)}
                </Text>
              </View>
            </View>
          );
      ...
      >
        ...
        <Pressable
          ...
          style={{
            ...
            borderWidth: 1,
            borderColor: isAwaitingResponse
              ? '#33FF33'
              : input.trim() && !composerDisabled
                ? '#20C20E'
                : '#0F2B0F',
          }}
        >
          {isAwaitingResponse ? (
            <Text style={{ color: '#33FF33', fontFamily: 'Geist Mono', fontSize: 9 }}>WAIT</Text>
          ) : (
