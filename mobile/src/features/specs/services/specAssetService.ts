...
export function normalizeSpecIdea(value: unknown, endpointId = ''): SpecIdea {
  ...
  return {
    ...
    targetProjectId: asOptionalString(field(raw, 'target_project_id', 'targetProjectId')),
    targetProjectName: asOptionalString(field(raw, 'target_project_name', 'targetProjectName')),
    targetResourceId: asOptionalString(field(raw, 'target_resource_id', 'targetResourceId')),
    targetResourceName: asOptionalString(field(raw, 'target_resource_name', 'targetResourceName')),
    body: asString(field(raw, 'body')),
...
function ideaPayload(input: UpdateSpecIdeaInput & { id?: string; body?: string }): Raw {
  ...
  return {
    ...
    target_project_id: input.targetProjectId ?? targetAgent?.project_id,
    target_project_name: input.targetProjectName ?? targetAgent?.project_path,
    target_resource_id: input.targetResourceId ?? targetAgent?.id,
    target_resource_name: input.targetResourceName ?? targetAgent?.name,
    notes: input.notes,
